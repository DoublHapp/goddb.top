...
<template>
  <article class="post-row">
    ...
    <div class="post-content">
      ...
      <h3><RouterLink :to="`/blog/${post.slug}`">{{ post.title[locale] }}</RouterLink></h3>
      <p>{{ post.excerpt[locale] }}</p>
      <div class="post-tags"><span v-for="tag in post.tags" :key="tag">#{{ tag }}</span></div>
    </div>
    <RouterLink :to="`/blog/${post.slug}`" class="row-arrow" :aria-label="t.common.read"><ArrowRight :size="21" /></RouterLink>
  </article>
</template>
